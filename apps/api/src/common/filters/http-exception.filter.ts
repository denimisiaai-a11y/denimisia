...
export class HttpExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger('ExceptionFilter');
    ...
    const body: ErrorBody = {
      ...
      requestId,
      ...(mapped.details !== undefined && !isProd() ? { details: mapped.details } : {}),
    };
    ...
    if (mapped.status >= 500) {
      ...
      this.logger.warn(`${request.method} ${request.url} → ${mapped.status} ${mapped.error}: ${mapped.message}`);
    }
  ...
  private map(exception: unknown): {
    ...
    if (exception instanceof HttpException) {
      ...
      if (typeof res === 'string') {
        return { status: exception.getStatus(), error: this.httpErrorName(exception.getStatus()), message: res };
      }
      const obj = res as { message?: string | string[]; error?: string };
      const msg = Array.isArray(obj.message) ? obj.message.join(', ') : (obj.message ?? exception.message);
      return {
    ...
    if (exception instanceof Prisma.PrismaClientValidationError) {
      return { status: 400, error: 'Bad Request', message: 'Invalid query parameters' };
    }
    ...
    if (exception instanceof Error) {
      ...
      if (err.type === 'entity.too.large') {
        return { status: 413, error: 'Payload Too Large', message: 'Request body exceeds maximum size' };
      }
      if (err.type === 'entity.parse.failed') {
        return { status: 400, error: 'Bad Request', message: 'Malformed request body' };
      }
    }
    ...
    const devDetail = exception instanceof Error
      ? { name: exception.name, message: exception.message }
      : { value: String(exception) };
    return {
      status:  HttpStatus.INTERNAL_SERVER_ERROR,
      error:   'Internal Server Error',
      message: isProd() ? 'An unexpected error occurred.' : (exception instanceof Error ? exception.message : String(exception)),
    ...
    switch (err.code) {
      ...
      default:
        return {
          status:  500,
          error:   'Internal Server Error',
          message: prod ? 'Database error.' : `Prisma ${err.code}: ${devDetail}`,
        };
    }
  }
