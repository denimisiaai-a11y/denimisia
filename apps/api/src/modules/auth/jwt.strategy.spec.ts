...
function makeDeps(opts: {
  ...
  const redis = {
    get: jest.fn((key: string) => {
      ...
      if (key.startsWith('auth:user:')) return Promise.resolve(opts.cachedUser ?? null);
...
describe('JwtStrategy', () => {
  ...
  it('returns user when token version matches and user is active', async () => {
    ...
    const payload: JwtPayload = { sub: 'user-1', email: 'a@b.com', role: 'CUSTOMER', tv: 7 };
    const result = await strategy.validate(payload);
    expect(result).toEqual({ id: 'user-1', email: 'a@b.com', role: 'CUSTOMER' });
  });
  ...
  it('rejects when token version is stale', async () => {
    ...
    const strategy = new JwtStrategy(config, prisma, redis);
    const payload: JwtPayload = { sub: 'user-1', email: 'a@b.com', role: 'CUSTOMER', tv: 3 };
  ...
  it('rejects when user is soft-deleted', async () => {
    ...
    const strategy = new JwtStrategy(config, prisma, redis);
    const payload: JwtPayload = { sub: 'user-1', email: 'a@b.com', role: 'CUSTOMER', tv: 0 };
    await expect(strategy.validate(payload)).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('rejects when sub is missing', async () => {
    const { config, prisma, redis } = makeDeps();
    const strategy = new JwtStrategy(config, prisma, redis);
    const payload = { sub: '', email: 'a@b.com', role: 'CUSTOMER', tv: 0 } as JwtPayload;
    await expect(strategy.validate(payload)).rejects.toThrow(UnauthorizedException);
  });
});
